...
def run_continuous_batch_learning(
    model,
    regression_model,
    ...
    """
    Perform batch-mode Active Learning in continuous parameter space for
    a given model generating the true data and a give regression model.
    The algorithm generates initial data automatically using either a
    random approach or using the model-free GSx approach.
    ...
    acquissition_function : str, optional
        Acquisition function. Can also be a Callable. Then in custom_acfn_input
        it needs to be specified which input the custom function takes.
        The Callable should only take the input in the described order.
        The default is 'ideal'.
    opt_method : str, optional
        The method to optimize the acquisition function. Choose from 'lbfgs'
        and 'PSO'. The default is 'PSO'.
    pool : nd_array, None, optional
        Array containing pool of data that is used for testing. For 'None' a
        grid with 100 points in each dimension is created. The default is 'None'.
    batch_size : int, optional
        Batch size for Active Learning. The model is updated only with true
    ...
    n_jobs : int, optional
        Number of cores to use for parallel evaluation of PSO. Currently not used,
        PSO runs only in serial mode. The default is 1.
    ...
    pso_options : dict, optional
        ...
        ['c1': 0.5, 'c2': 0.3, 'w': 0.9, 'p':dimensions*10, 'i':200]. c1 and c2
        are swarm parmeters, w is the inertia, p gives the number
        of particles, i is the number of iterations. The default is 'None'.
    fictive_noise_level : float, optional
        Noise level for non-GPR models used for assuming predictions as
        true values to enable batch-wise learning.
    calculate_test_metrics : bool, optional
        Can be used for testing Active Learning algorithms for known models.
        Test metrics are calculated automatically. If 'False' no test metrics are
        calculated and the AL runs in deployement mode.
    custom_args_input : list, optional
        List of arguments which a custom acquisition function takes. It is important
        that the callable takes the arguments
        ...
        poly_x: sklearn poly_transformer. Must be given if a linear regression model
        should be used with the acquisition function

        The callable must return the value of the acquisition function as a float
        or as a numpy_array of floats.
    ...
    results : pd_DataFrame, optional
        Pandas DataFrame containing the columns: 'm', 'mean_mse_test', 'max_observation'.
        Data is stored for each active learning step, where 'm' gives the number of data
        used for training, 'mean_mse_test' the MSE of the test set and 'max_observation'
        the maximum value observed so far. Only returned when calculate_test_metrics is 'True'.
    ...
    # Fit initial model
    regression_model = utils.fit_model(
        sample_x, observation_y, regression_model, poly_transformer
    )

    scores_train = np.zeros((active_learning_steps + 1, 3))
    max_value = np.zeros((active_learning_steps + 1, 1))
    n_observations = np.linspace(
        initial_samples,
        initial_samples + (active_learning_steps) * batch_size,
        active_learning_steps + 1,
    )

    mean, _ = utils.make_prediction(sample_x, regression_model, poly_transformer)
    ...
    if calculate_test_metrics:
        ...
        mean, _ = utils.make_prediction(pool, regression_model, poly_transformer)
    ...
    for i in range(active_learning_steps):
        ...
        for j in range(batch_size):
            ...
            mean_new, std_new = utils.make_prediction(
                new_x, regression_model, poly_transformer, fictive_noise_level
            )
    ...
    if calculate_test_metrics:
        results = utils.results_to_df(
            n_observations, scores_train, max_value, scores_test
        )
    else:
...
def run_batch_learning(
    ...
    """
    Perform batch-mode Active Learning for a discrete parameter space. The algorithm picks
    automatically initial data points from a given pool of discrete possible data points
    for evaluation using either a random approach or the model-free GSx method.
    ...
    active_learning_steps : int, optional
        Number of active learning steps to perform. The default is 10.
    lim : list, optional
        Boundaries for model evaluation. Only used when pool=None.
        The default is [-1,1].
    alpha : float
        Hyperparameter for acquisition function. The default is 0.
    random_state: int, optional
    ...
    initialization : str, optional
        ...
        'random' uses Latin Hypercube sampling to generate the initial dat points.
        'GSx' draws randomly the first data point and then uses the model-free GSx method
        to sample the other initial data points. The default value is 'random'.
    test_set : nd_array, optional
        Array containing explicit data points for testing. If it is 'None' all unsampled
        data points in the pool will be used for testing.
        The default value is 'None'.
    calculate_test_metrics : bool, optional
        Can be used for testing Active Learning algorithms for known models.
        Test metrics are calculated automatically. If 'False' no test metrics
        are calculated and the AL runs in deployement mode.
    ...
    sample_x : nd_array, optional
        Array that contains the sampled data points (in terms of their features).
        Only returned when return_samples is True.
    ...
    if initialization == "random":
        ...
        initial_data, _ = run_batch_learning(
            model,
            regression_model,
            acquisition_function="GSx",
            pool=pool,
    ...
    if calculate_test_metrics:
        ...
        mean_test, _ = utils.make_prediction(
            test_set, regression_model, poly_transformer
        )
    ...
    for i in range(active_learning_steps):
        ...
        for j in range(batch_size):
            if j != 0:
                ...
                mean, std = utils.make_prediction(
                    pool, regression_model, poly_transformer
                )
            ...
            if acquisition_function == "qbc":
                ...
                for _ in range(alpha):
                    train_index = rng.randint(
                        0, len(estimated_sample_x), len(estimated_sample_x)
                    )
                    if isinstance(reg_model_pure, LinearRegression):
                        estimated_sample_x_poly = poly_transformer(estimated_sample_x)
                        regression_model.fit(
                            estimated_sample_x_poly[train_index],
                            estimated_observation_y[train_index],
                        )
                    else:
                        regression_model.fit(
                            estimated_sample_x[train_index],
                            estimated_observation_y[train_index],
                        )
            ...
            # Update the model
            mean_new, std_new = utils.make_prediction(
                pool[index], regression_model, poly_transformer, fictive_noise_level
            )
            estimated_observation_new = mean_new + rng.normal(0, std_new, size=1)
        ...
        mean, std = utils.make_prediction(pool, regression_model, poly_transformer)
        scores_train[i + 1, ...] = utils.calculate_errors(
            observation_y, mean[data_indices]
        )
        max_value[i + 1, 0] = np.max(observation_y)
