    ...
    def evaluate(self, X, noise=0):
        ...
        if self.n_features == 1:
            X = X.reshape(
                self.n_features,
                *[
                    int(X.shape[0] ** (1 / self.n_features))
                    for _ in range(self.n_features)
                ]
            )
            y = self.h * self.n_features + np.sum(
                [(x**2 - self.h * np.cos(2 * np.pi * x)) for x in X], axis=0
            )
        else:
            y = self.h * self.n_features + np.sum(
                [(x**2 - self.h * np.cos(2 * np.pi * x)) for x in X], axis=-1
            )
...
class PolyModel:
    ...
    def _internal_evaluation(self, grid):
        ...
        y = (
            np.sum(poly_features * np.array(self.weights.iloc[0]), axis=1)
            + self.weights.iloc[0]["const"]
        )
        return y
