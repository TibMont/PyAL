...
def GSy_multi(x, y_sample, model, aggregation_function, poly_x, *args, **kwargs):
    if len(x.shape) == 1:
        x = x.reshape(1,-1)
...
def iGS_multi(x, x_sample, y_sample, model, aggregation_function, poly_x=None, *args, **kwargs):
    gsx = GSx_multi(x,x_sample)
    gsy = GSy_multi(x, y_sample, model, aggregation_function, poly_x, *args, **kwargs)

    min_dist = gsx*gsy
    return -min_dist

def IDEAL_multi(x, x_samples, model, y_true, lim, aggregation_function, alpha=1, poly_x=None, tol=1e-8, *args, **kwargs):
    if len(x.shape) == 1:
        n_pool = 1
        x = x.reshape(1,-1)
    else:
...
def UCB_multi(x, model, aggregation_function, alpha=0.5, *args, **kwargs):
    if len(x.shape) == 1:
        x = x.reshape(1,-1)
        n_pool = 1
    else:
...
def EI_multi(x, model, aggregation_function, opt, alpha=0.5, max=True, *args, **kwargs):
    if len(x.shape) == 1:
        x = x.reshape(1,-1)
        n_pool = 1
    else:
...
def QBC_multi(x, models, aggregation_function, poly_x, *args, **kwargs):
    if len(x.shape) == 1:
        x = x.reshape(1,-1)
        n_pool = 1
    else:
...
def UIDAL_multi(x, x_samples, model, aggregation_function, alpha=1, *args, **kwargs):
    #print(grid.shape)
    #print(grid)
    #print(uncertainty)
    if len(x.shape) == 1:
...
def SGSx_multi(x, x_sample, model, aggregation_function, alpha=0.5, *args, **kwargs):
    if len(x.shape) == 1:
        n_pool = 1
        x = x.reshape(1,-1)
    else:
...
def std_multi(x, model, aggregation_function, *args, **kwargs):
    if len(x.shape) == 1:
        n_pool = 1
        x = x.reshape(1,-1)
    else:
