...
def step_discrete(
    ...
    if acquisition_function == "poi":
        acquisition = POI(
            mean, std, opt=np.max(estimated_observation_y), max=True, alpha=alpha
        )
        ...
        raise KeyError(
            'Acquisition function "{}" not implemented'.format(acquisition_function)
        )
    return acquisition
    ...
    if acquisition_function == "random":
        ...
        if acquisition_function == "ei":
            res = minimize(
                EI_con,
                x0=x0,
                args=(regression_model, np.max(estimated_observation_y), alpha),
                bounds=lim_t,
            )
            ...
            res = minimize(UCB_con, x0=x0, args=(regression_model, alpha), bounds=lim_t)
        elif acquisition_function == "ideal":
            res = minimize(
                IDEAL_con,
                x0=x0,
            ...
            res = minimize(
                UIDAL_con,
                x0=x0,
                args=(estimated_sample_x, regression_model, alpha),
                bounds=lim_t,
            )
        elif acquisition_function == "std":
            res = minimize(std_con, x0=x0, args=(regression_model), bounds=lim_t)
        elif acquisition_function == "GSx":
            res = minimize(GSx_con, x0=x0, args=(estimated_sample_x), bounds=lim_t)
        elif acquisition_function == "GSy":
            res = minimize(
                GSy_con,
                x0=x0,
                args=(estimated_observation_y, regression_model, poly_x),
                bounds=lim_t,
            )
        elif acquisition_function == "iGS":
            res = minimize(
            ...
            res = minimize(QBC_con, x0=x0, args=(models, poly_x), bounds=lim_t)
        elif callable(acquisition_function):
            custom_args = []
            ...
            res = minimize(acquisition_function, x0=x0, args=custom_args, bounds=lim_t)
    ...
    # Simple Particle Swarm Optimization
        ...
        if acquisition_function == "ei":
            cost, new_x = optimizer.optimize(
                ...
                opt=np.max(estimated_observation_y),
                alpha=alpha,
            )
        elif acquisition_function == "poi":
            cost, new_x = optimizer.optimize(
    ...
    **kwargs
    ...
    if acquisition_function == "random":
        ...
        if acquisition_function == "ei":
            ...
            res = minimize(GSx_multi, x0=x0, args=(estimated_sample_x), bounds=lim_t)
        elif acquisition_function == "GSy":
            res = minimize(
                GSy_multi,
                x0=x0,
            ...
            res = minimize(
                QBC_multi,
                x0=x0,
                args=(s_models, aggregation_function, poly_x, *optargs),
                bounds=lim_t,
            )
        elif callable(acquisition_function):
            custom_args = []
    ...
    # Simple Particle Swarm Optimization
        ...
        if acquisition_function == "ei":
            ...
            cost, new_x = optimizer.optimize(
                ...
                x_samples=estimated_sample_x,
                model=regression_models,
                y_true=estimated_observation_y_aggregated,
                lim=lim,
                aggregation_function=aggregation_function,
                alpha=alpha,
                poly_x=poly_x,
