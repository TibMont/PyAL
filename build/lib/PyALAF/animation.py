...
def create_animation(
    ...
    def plot_anim(
        ...
        ax1.plot(
            sample_x, observation_y, "ko", markerfacecolor="white", label="Observation"
        )
        ax1.set_ylabel("$f(x)$")
        if legend:
        ...
        if plot_std:
            ax1.fill_between(
                grid.reshape(n_data), mean - std * 3, mean + std * 3, alpha=0.2
            )
    ...
    def animation_function(
        ...
        plot_anim(
            sample_x_collect[frame],
            observation_y_collect[frame],
            grid_simple,
            mean_collect[frame],
            std_collect[frame],
            acquisition_collect[frame],
            x_max_collect[frame],
            acquisition_max_collect[frame],
...
def create_animation_continuous(
    ...
    def plot_anim(
        ...
        if plot_std:
            ax1.fill_between(
                grid.reshape(n_data_plot), mean - std * 3, mean + std * 3, alpha=0.2
            )
