...
from PyALAF.optimize_step import step_discrete
...
def QBC_multi(x, models, aggregation_function, poly_x, **kwargs):
    ...
    for i in range(mean_qbc_individual.shape[1]):
        mean_qbc[i] = aggregation_function(mean_qbc_individual[:, i, :], x, **kwargs)
...
def run_batch_learning_multi(
    ...
    **kwargs
    ...
    """
    Perform batch-mode Active Learning for a discrete parameter space.
    The algorithm picks automatically initial data points from a given
    pool of discrete possible data points for evaluation using either
    a random approach or the model-free GSx method.
    ...
    batch_size : int, optional
        ...
        true data when a batch is completed. The default value is 1.
    ...
    initialization : str, optional
        Initialization method for generating initial data. Choose from 'random'
        and 'GSx'. 'random' uses Latin Hypercube sampling to generate the initial
        data points. 'GSx' draws randomly the first data point and then uses
    ...
    if isinstance(regression_models, list):
        if len(regression_models) != len(models):
            raise Exception(
                "Inconsistent number of data and regression models: {}, {}".format(
                    len(models), len(regression_models)
                )
            )
    else:
        regression_models = [
            copy.deepcopy(regression_models) for i in range(len(models))
        ]

    reg_model_ini = []
    for regression_model in regression_models:
        reg_model_ini.append(clone(regression_model))

    reg_models_pure = []
    for regression_model in regression_models:
        reg_models_pure.append(
            utils.check_model(regression_model, acquisition_function)
        )
    ...
    n_observations = np.linspace(
        len(rand_num),
        len(rand_num) + (active_learning_steps) * batch_size,
    ...
    for i in range(n_models):
        ...
        scores_train_individual[i, 0, ...] = utils.calculate_errors(
            observation_y[i], mean[i][data_indices]
        )
    ...
    if calculate_test_metrics:
        ...
        if extra_test_set == False:
            ...
            scaled_test_set = scaled_pool[mask_indices]
            # print(mask_indices)
            # print(y_true)
    ...
    for a in range(active_learning_steps):
        ...
        estimated_sample_x_scaled = sample_x_scaled.copy()
        ...
        for j in range(batch_size):
            ...
            if acquisition_function == "qbc":
                ...
                for i in range(n_models):
                    ...
                    for _ in range(alpha):
                        ...
                        if isinstance(reg_models_pure[i], LinearRegression):
                            ...
                            estimated_sample_x_poly = poly_transformer.transform(
                                estimated_sample_x_scaled
                            )
                            regression_models[i].fit(
                            ...
                            poly_x = None
                            regression_models[i].fit(
                                estimated_sample_x_scaled[train_index],
                                estimated_observation_y[i][train_index],
                            )
                        alpha_models.append(copy.deepcopy(regression_model))
                    S_models.append(alpha_models)
                acquisition = QBC_multi(
                    pool, S_models, aggregation_function, poly_x, **kwargs
                )
            else:
                acquisition = step_discrete(
                    ...
                    alpha,
                    mean_train_aggregated,
                    std_train_aggregated,
                    n_data,
                    data_indices,
                    pool,
                    custom_acfn_input,
            ...
            estimated_observation_new = np.zeros(
                (n_models, len(estimated_x_max.reshape(1, -1)))
            )
            ...
            estimated_observation_y_aggregated = np.hstack(
                [
                    estimated_observation_y_aggregated,
        ...
        x_max_scaled = scaled_pool[batch_indices]
        sample_x = np.vstack([sample_x, x_max])

        # Sorting is necessary, since the order of the samples is relevant for NN with lbfgs
        idx_sort = np.argsort(sample_x[:, 0], axis=0)  #
        rng_shuggle = np.random.default_rng(12345)
        rng_shuggle.shuffle(idx_sort)
        # print('Idx:', idx_sort)
        sample_x_sorted = sample_x[idx_sort]  #
        sample_x_scaled = np.vstack([sample_x_scaled, x_max_scaled])
        sample_x_scaled_sorted = sample_x_scaled[idx_sort]  #

        if single_update:
            if calculate_test_metrics:
                result_dict = {}
                result_dict["aggregated"] = utils.results_to_df(
                    n_observations[0],
                    scores_train[0],
                    max_value[0],
                    scores_test[0],
                ...
                for i in range(n_models):
                    result_dict["model_" + str(i)] = utils.results_to_df(
                        n_observations[0],
                        scores_train_individual[i, 0],
                        max_value_individual[i, 0],
                        scores_test_individual[i, 0],
                        single_update=True,
                    )
            else:
                result_dict = {}
                result_dict["aggregated"] = utils.results_to_df(
                    n_observations[0], scores_train[0], max_value[0], single_update=True
                )
                for i in range(n_models):
                    result_dict["model_" + str(i)] = utils.results_to_df(
                        n_observations[0],
        ...
        observation_y_sorted = np.zeros_like(observation_y)
        for i in range(n_models):
            observation_y_sorted[i, ...] = observation_y[i][idx_sort]  #
        observation_y_aggregated_sorted = observation_y_aggregated[idx_sort]  #
        ...
        regression_models = []
        for regression_model in reg_model_ini:
            regression_models.append(clone(regression_model))

        # print('Sample')
        # print(sample_x_scaled)
        ...
        for i in range(n_models):
            regression_models[i] = utils.fit_model(
                sample_x_scaled_sorted,
                observation_y_sorted[i],
                regression_models[i],
                poly_transformer,
            )
            mean[i, ...], std[i, ...] = utils.make_prediction(
    ...
    if calculate_test_metrics:
        ...
        for i in range(n_models):
            result_dict["model_" + str(i)] = utils.results_to_df(
                n_observations,
                scores_train_individual[i],
                max_value_individual[i],
                scores_test_individual[i],
            )
    else:
        result_dict = {}
        result_dict["aggregated"] = utils.results_to_df(
            n_observations, scores_train, max_value
        )
        for i in range(n_models):
            result_dict["model_" + str(i)] = utils.results_to_df(
                n_observations, scores_train_individual[i], max_value_individual[i]
            )
