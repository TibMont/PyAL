...
def iGS_multi(
    x, x_sample, y_sample, model, aggregation_function, poly_x=None, *args, **kwargs
):
...
def IDEAL_multi(
    ...
    **kwargs
    ...
    for i in range(ny):
        ...
        ff += np.sum((vk) * ((Yhat[:, i, np.newaxis] - y_true) ** 2), axis=1) / dY2
...
def UCB_multi(x, model, aggregation_function, alpha=0.5, *args, **kwargs):
    ...
    for i in range(len(model)):
        mean_individual[i], uncertainty_individual[i] = model[i].predict(
            x, return_std=True
        )
