...
def run_continuous_batch_learning_multi(
    models,
    ...
    **kwargs
    ...
    """
    Multi-objective batch-mode Active Learning in continuous parameter space.
    This method is similar to 'run_continuous_batch_learning' but also enables
    to take into account multiple objective functions.
    Multiple objectives are handeled by an aggregation function, which combines
    the individual output for each data point of each objective into a single output value.
    The aggregation functions needs to be provided customly.
    For each objective function it is also possible to use an individual model.
    E.g. we can use Gaussian Process Regression for objective 1 and
    Linear Regression for objective 2.

    The active learning acquisition functions are modified in order to also take
    the aggregation function as an argument. Therefore, active learning
    acquisition functions from outside this module are not compatible.

    The algorithm generates initial data automatically using either a random
    approach or using the model-free GSx approach.
    ...
    regression_models: sklearn model, list
        scikit-learn regression models. Either a single model can be given or a
        list of models with a length corresponding to the number of objectives.
    acquissition_function : str, optional
        Acquisition function. The default is 'ideal'.
    opt_method : str, optional
        The method to optimize the acquisition function. Choose from 'lbfgs' and 'PSO'.
        The default is 'PSO'.
    pool : nd_array, None, optional
        Array containing pool of data for testing. For 'None' a grid with 100 points in
        each dimension is created.
        This is only used when 'calculate_test_metrics' is True. The default is 'None'.
    batch_size : int, optional
        Batch size for Active Learning. The model is updated only with true data when a
        batch is completed. The default value is 1.
    ...
    pso_options : dict, optional
        ...
        pt_method is 'PSO'. For 'None' default values are used.
        ['c1': 0.5, 'c2': 0.3, 'w': 0.9, 'p':dimensions*10, 'i':200]. c1 and c2 are swarm parmeters,
        w is the inertia, p gives the number of particles, i is the number of iterations.
        The default is 'None'.
    fictive_noise_level : float, optional
        Noise level for non-GPR models used for assuming predictions as true
        values to enable batch-wise learning.
    ...
    **kwargs : various, optional
    ...
    -------
    samples : nd_array
        Numpy array which contains the unscaled features for data points in the order
        in which they were selected.
    results : pd_DataFrame
        Pandas DataFrame containing the columns: 'm', 'mean_MSE_train',
        'mean_MAE_train', 'mean_MaxE_train', 'mean_MSE_test', 'mean_MAE_test',
        'mean_MaxE_test', 'max_observation'.
        Data is stored for each active learning step,
        where 'm' gives the number of data used for training, 'mean_MSE_train'
        and 'mean_MSE_test' the MSE of the training and test set, respectively,
        and 'max_observation' the maximum value observed so far. MAE corresponds
        to the Mean Absolute Error and MaxE to the maximum absolute error.
    ...
    if initialization == "random":
        ...
        if isinstance(initial_samples, int):
            sample_x, _, _ = run_continuous_batch_learning_multi(
                models,
                aggregation_function,
                regression_models,
            ...
            raise Exception(
                "initial_samples must be an integer for initialization method GSx"
            )
    ...
    for a in range(active_learning_steps):
        ...
        # Calculate metrics
        if verbose:
            print("Active learning step: {}".format(a))
